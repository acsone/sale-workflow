...
class SaleOrder(models.Model):
    ...
    def _check_call_of_link_to_valid_blanket(self):
        for rec in self:
            if (
                ...
                or rec.blanket_order_id.state != "sale"
            ):
                continue
            expected_delivery_date = rec.date_order or rec.commitment_date
    ...
    def _check_blanket_order_state(self):
        for order in self:
            ...
            if (
                order.order_type == "call_off"
                and order.blanket_order_id.state != "sale"
            ):
                raise ValidationError(
                    _(
    ...
    def action_view_call_off_orders(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "sale.sale_order_action_view_order_tree"
        )
    ...
    def _get_single_blanket_order_candidates_domain(self):
        ...
        return [
            ...
            ("order_type", "=", "blanket"),
            ("state", "=", "sale"),
            ("blanket_validity_start_date", "<=", self.date_order),
            ("blanket_validity_end_date", ">=", self.date_order),
        ]
